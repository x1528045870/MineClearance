...
public class AIGamePanel2 extends JPanel {
    ...
    public AIGamePanel2(int xCount, int yCount, int mineCount) {
    ...
    private void right(Btn btn, int i, int j) {
        if(mineField[i][j].getStatus()==GridStatus.Covered){
            ...
            AIDialog2.controller.getScoreBoard().update(score,mistake,remainMineCount);
        }
    }
    ...
    public void AICheck(){
        if (remainMineCount!=0){
            ...
            int t=0;int Max=0;;boolean check=false;
            for (int z=0;z<5;z++){
             a[z]=random.nextInt(x);
             b[z]=random.nextInt(y);
             int h=checkAround(a[z],b[z]);
             if(h>Max) {Max=h;t=z;}
             if(z==4&&mineField[a[t]][b[t]].getStatus()!=GridStatus.Covered){
                 z=0;Max=0;
             }
            }
            int i=a[t];int j=b[t];
            ...
            if (z){ if(mineField[i][j].getStatus()==GridStatus.Covered){
                ...
                mineField[i][j].setStatus(GridStatus.Flag);
                AIDialog2.controller.getScoreBoard().AIUpdate(AIScore,AIMistake);
            }
            }
            if(!z){
                if(mineField[i][j].getStatus()==GridStatus.Covered) {
    ...
    public int checkAround(int i,int j){
        ...
        if(i>0&&mineField[i-1][j].getStatus()==GridStatus.Clicked&&!mineField[i-1][j].isHasMine()){
            t++;
        }
        if(i>0&&j>0&&mineField[i-1][j-1].getStatus()==GridStatus.Clicked&&!mineField[i-1][j-1].isHasMine()){
            t++;
        }
        if(i>0&&j<y-1&&mineField[i-1][j+1].getStatus()==GridStatus.Clicked&&!mineField[i-1][j+1].isHasMine()){
            t++;
        }
        if(j>0&&mineField[i][j-1].getStatus()==GridStatus.Clicked&&!mineField[i][j-1].isHasMine()){
            t++;
        }
        if(j<y-1&&mineField[i][j+1].getStatus()==GridStatus.Clicked&&!mineField[i][j+1].isHasMine()){
            t++;
        }
        if(i<x-1&&j>0&&mineField[i+1][j-1].getStatus()==GridStatus.Clicked&&!mineField[i+1][j-1].isHasMine()){
            t++;
        }
        if(i<x-1&&j<y-1&&mineField[i+1][j+1].getStatus()==GridStatus.Clicked&&!mineField[i+1][j+1].isHasMine()){
            t++;
        }
        if(i<x-1&&mineField[i+1][j].getStatus()==GridStatus.Clicked&&!mineField[i+1][j].isHasMine()){
            t++;
        }
        return t;
    }
        ...
        aBtn.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                ...
                if(e.getButton() == MouseEvent.BUTTON1) {
                    AIStop=true;aBtn.setEnabled(false);
                }
    ...
    public void addSkin(){
        ...
        pBtn.i=x;
        pBtn.j=y;
        pBtn.setBounds(y* gridSize, buttonSize*2, buttonSize, buttonSize);
    ...
    public void changeSkin(){
        ImageIcon image2=new ImageIcon("img\\cute02.jpg");
        Image g = image2.getImage();
        g = g.getScaledInstance(Grid.gridSize, Grid.gridSize,Image.SCALE_DEFAULT);
        image2.setImage(g);
        for (int i = 0; i < x; i++) {
            for (int j = 0; j < y; j++) {
                face[i][j].setIcon(image2);
    ...
    public void iniSkin(){
        ImageIcon image2=new ImageIcon("img\\篮球框.jpg");
        Image g = image2.getImage();
        g = g.getScaledInstance(Grid.gridSize, Grid.gridSize,Image.SCALE_DEFAULT);
        image2.setImage(g);
        for (int i = 0; i < x; i++) {
