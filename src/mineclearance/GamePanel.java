...
public class GamePanel extends JPanel {
    ...
    public GamePanel(int xCount, int yCount, int mineCount,int clickTimes) {
    ...
    public GamePanel(int [][]message){
        this.x=message[0][0];
        this.y=message[0][1];
        this.mineCount=message[0][2];
        this.clickTimes=message[0][3];
        this.remainMineCount=message[7][3];
        ...
        this.setLayout(null);
        this.setBackground(Color.WHITE);
        this.setSize(Grid.gridSize * x+buttonSize, Grid.gridSize * y);
        addXRay();
        addRecover();
        addSaveIn();
        addMeet0();
    ...
    private void iniBtn(int xCount,int yCount) {
        ...
        for (int i = 0; i < xCount; i++) {
            for (int j = 0; j < yCount; j++) {
                ...
                MyMouseAdapter myMouseAdapter = new MyMouseAdapter(btn);
    ...
    public void iniGame(int [][]message){
        ...
        for (int t=0;t<message[5].length;t++){
            ...
            else {  ImageIcon image3=new ImageIcon("img\\打铁声声.png");
    ...
    private void right(Btn btn, int i, int j) {
        if(mineField[i][j].getStatus()==GridStatus.Covered){
               lastStep[0]=i;
               lastStep[1]=j;
               lastStep[2]=GameDialog.controller.getOnTurnPlayerNum();
               if(mineField[i][j].isHasMine()){
                   Sound in = new Sound("music\\进球.wav");
                   in.start(false);
                   try {
                   ...
                   image2.setImage(img);
                   face[i][j].setIcon(image2);
                GameDialog.controller.getOnTurnPlayer().addScore();
                //GameDialog.controller.setRemainMineCount(GameDialog.controller.getRemainMineCount()-1);
                   remainMineCount--;
            }
            if(!mineField[i][j].isHasMine()){
                Sound iron = new Sound("music\\打铁.wav");
                iron.start(false);
                try {
                    TimeUnit.SECONDS.sleep(1/2);
                } catch (InterruptedException e) {
                ...
                image3.setImage(img);
                face[i][j].setIcon(image3);
            GameDialog.controller.getOnTurnPlayer().addMistake();}
                GameDialog.controller.getOnTurnPlayer().cutClicks();
            mineField[i][j].setStatus(GridStatus.Flag);

            if(GameDialog.controller.getOnTurnPlayer().getClicks()==0||remainMineCount==0){
                GameDialog.controller.nextTurn();}
            GameDialog.controller.getScoreBoard().update();
            GameDialog.controller.victory();
    ...
    private void left(Btn btn,int i,int j) {
        if(mineField[i][j].getStatus()==GridStatus.Covered) {
            ...
            if(mineField[i][j].isHasMine()){
                ...
                GameDialog.controller.getOnTurnPlayer().costScore();
                remainMineCount--;
                //GameDialog.controller.setRemainMineCount(GameDialog.controller.getRemainMineCount()-1);
            }
            GameDialog.controller.getOnTurnPlayer().cutClicks();
    ...
    public void checkPanel(int i,int j){//避免首次触雷by xed
        int turn=GameDialog.controller.getTurns();
        int Clicks=GameDialog.controller.getClick();

        if(mineField[i][j].isHasMine()&&turn==0&&GameDialog.controller.getOnTurnPlayer().getClicks()==GameDialog.controller.getClickTimes()){
            iniPanel(x,y,mineCount);GameDialog.controller.setTurns(0);
            checkPanel(i,j);
        }
    }//看看是否339雷
    ...
    public void addBack(){
        ...
        hui.setVisible(true);
        setImage("img\\logo\\Harden.jpg",hui);
        hui.setToolTipText("悔棋");
    ...
    public void back(){
        ...
        if (mineField[i][j].getStatus()==GridStatus.Clicked){
            ...
            if(lastStep[2]==1){
                if(GameDialog.controller.getP1().getClicks()==0){
                    GameDialog.controller.goBackTurn();
                }
                GameDialog.controller.getP1().addClicks();
                if (mineField[i][j].isHasMine()){
                    GameDialog.controller.getP1().addScore();
                remainMineCount++;}
            }
                ...
                GameDialog.controller.getP2().addClicks();
                if (mineField[i][j].isHasMine()){
                    GameDialog.controller.getP2().addScore();
                    remainMineCount++;
                }
            }
        }
            ...
            if(lastStep[2]==1){
                ...
                GameDialog.controller.getP1().addClicks();
                if (mineField[i][j].isHasMine()){
                    GameDialog.controller.getP1().costScore();
                    remainMineCount++;
                }
                else {
                    GameDialog.controller.getP1().cutMistake();
                }
            }
            else if(lastStep[2]==2){
                if(GameDialog.controller.getP1().getClicks()==0){
                ...
                GameDialog.controller.getP2().addClicks();
                if (mineField[i][j].isHasMine()){
                    GameDialog.controller.getP2().costScore();
                    remainMineCount++;
                }
                else {
                    GameDialog.controller.getP2().cutMistake();
                }
            }
        }
        GameDialog.controller.getScoreBoard().update();
    ...
    public void addXRay(){
        ...
        XRay.setVisible(true);setImage("img\\logo\\rose.jpg",XRay);
        XRay.setToolTipText("透视");
        this.add(XRay);
        ...
        XRay.addMouseListener(new MouseAdapter() {
           ...
           // public void mousePressed(MouseEvent e){
              //  XRay.setSize(120,120);
            //}
            //public void mouseReleased(MouseEvent e){
              //  if(e.getButton() == MouseEvent.BUTTON1) {XRay.setSize(buttonSize,buttonSize);}
            //}
    ...
    public void addMeet0(){
        ...
        meet0.setToolTipText("级联");
        setImage("img\\logo\\kyrie.jpg",meet0);
        this.add(meet0);
    ...
    public void addRecover(){
        ...
        setImage("img\\logo\\james.jpg",aBtn);
        aBtn.setToolTipText("左键盖上所有格子，右键翻开所有格子");
        this.add(aBtn);
        aBtn.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                /* 左键点击 */
                if(e.getButton() == MouseEvent.BUTTON1) {recover(x,y);
                    GameDialog.controller.init(GameDialog.controller.getP1(),GameDialog.controller.getP2());
    ...
    public void addNew(){
        ...
        setImage("img\\logo\\bryant.jpg",pBtn);
        pBtn.setToolTipText("重新开始一局游戏");
        this.add(pBtn);
    ...
    public void  recover(int xCount,int yCount){
        for (int i = 0;i <xCount ;i++){
            for(int j = 0;j < yCount; j++){
                {face[i][j].setVisible(true);
                    ...
                    if(mineField[i][j].getStatus()==GridStatus.Flag){
                        ImageIcon image3=new ImageIcon("img\\篮球框.jpg");
                        Image img = image3.getImage();
                        img = img.getScaledInstance(Grid.gridSize, Grid.gridSize,Image.SCALE_DEFAULT);
                        image3.setImage(img);
                        face[i][j].setIcon(image3);
            ...
            GameDialog.controller.setTurns(0);GameDialog.controller.getOnTurnPlayer().setClicks();
        }
    }//存档
    public void addSaveIn(){
        ...
        saveIn.setVisible(true);setImage("img\\logo\\curry.jpg",saveIn);
        saveIn.setToolTipText("存档");
        this.add(saveIn);
    ...
    public void saveIn(){
        ...
        try {
            assert fw != null;
            fw.write(GameDialog.controller.getP1().getUserName()+"\t"+GameDialog.controller.getP2().getUserName()+"\n");
        } catch (IOException e) {
            e.printStackTrace();
        }
        for(int i = 0;i<10;i++) {
    ...
    public void saveWhat(){//记录信息
        ...
        //记录回合数，计时,当前玩家，悔棋次数等
        message[7]=new int[4];
        message[7][0]=GameDialog.controller.getTurns();
        message[7][1]=GameDialog.controller.getOnTurnPlayerNum();
        message[7][2]=GameDialog.controller.getOnTurnPlayer().getClicks();//记录当前玩家的click次数
        message[7][3]=remainMineCount;
        message[8]=new int[3];
        message[9]=new int[3];
        //记录玩家信息
        message[8][0]=GameDialog.controller.getP1().getScore();
        message[9][0]=GameDialog.controller.getP2().getScore();
        message[8][1]=GameDialog.controller.getP1().getMistake();
        message[9][1]=GameDialog.controller.getP2().getMistake();
        message[8][2]=GameDialog.controller.getP1().getTeamCode();
        message[9][2]=GameDialog.controller.getP2().getTeamCode();
    }
    ...
    static class MyMouseAdapter extends MouseAdapter{
        JButton button;Sound dribble;
        public MyMouseAdapter(JButton button){
            this.button = button;
        }
        public void mouseEntered(MouseEvent e) {
             dribble= new Sound("music\\运球.wav");
            dribble.start(false);
            try {
                TimeUnit.SECONDS.sleep(1/2);
    ...
    private void setImage(String filepath,JButton button){
        ImageIcon image = new ImageIcon(filepath);
        Image img = image.getImage();
        img = img.getScaledInstance(buttonSize,buttonSize,Image.SCALE_DEFAULT);
        image.setImage(img);
